...
def estimate_mem(data_dict, in_feat, hidden_size, redundant_ratio, cluster_coefficient):	
	...
	for idx,(key, val) in enumerate(estimated_mem_dict.items()):
		if idx < len(estimated_mem_dict)-1:
			estimated_mem_list.append(estimated_mem_dict[key])
			print('  estimated memory/GB degree '+str(key)+': '+str(estimated_mem_dict[key])  ) 
		elif idx == len(estimated_mem_dict)-1:
			estimated_mem_list.append(estimated_mem_dict[key]/redundant_ratio[idx])
			print('  estimated memory/GB degree '+str(key)+': '+str(estimated_mem_dict[key])+ " / " +str(redundant_ratio[idx]) )
	print()
	print('estimated_mem_list [:-1]')
	print(estimated_mem_list[:-1])
	print('sum [:-1] = ', sum(estimated_mem_list[:-1]))
	print('mestimated_mem_list [-1]')
	print(estimated_mem_list[-1])
	...
	return estimated_mem_list, list(estimated_mem_dict.values())
	...
	for run in range(args.num_runs):
		...
		for epoch in range(args.num_epochs):
			...
			if args.load_full_batch:
				...
				file_name=r'/home/cc/dataset/fan_out_'+args.fan_out+'/'+args.dataset+'_'+str(2)+'_items.pickle'
...
def main():
	...
	# argparser.add_argument('--cluster-coefficient', type=float, default=0.085)
	...
	argparser.add_argument('--mem-constraint', type=float, default=72)
