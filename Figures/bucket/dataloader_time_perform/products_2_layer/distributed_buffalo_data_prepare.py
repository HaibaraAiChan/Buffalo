	...
	for run in range(args.num_runs):
		...
		for epoch in range(args.num_epochs):
			...
			if args.num_batch > 1:
				...
				# Save block_dataloader to a pickle file  
    
				 
				num_batches_per_gpu = len(block_dataloader) // args.num_batch  
				micro_batches = [[] for _ in range(args.num_batch )]  

				# 将元素分配到 num-batch 个列表  
				for i in range(len(block_dataloader)):  
					micro_batches[i % args.num_batch].append(block_dataloader[i])  # 使用模运算将元素分配到不同的列表  

				# 输出结果  
				# for index, batch in enumerate(micro_batches):  
				# 	print(f"list{index + 1}:", batch)
				# print('micro_batches ', micro_batches)
				print('len(micro_batches) ', len(micro_batches))
				...
				for i in range(len(micro_batches)):
					print('i ', i)
					with open(prefix+str(i)+'_micro_batch_block_dataloader.pkl', 'wb') as f:  
						pickle.dump(micro_batches[i], f)  
